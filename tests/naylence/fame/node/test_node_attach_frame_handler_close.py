...
@pytest.mark.asyncio
async def test_node_attach_frame_handler_no_longer_closes_connections():
    """Test that NodeAttachFrameHandler no longer handles authorization or closes connections"""
    ...
    # Verify that positive acknowledgment was sent (no authorization failure)
...
async def test_node_attach_frame_handler_still_works_successfully():
    ...
    # Wait a moment to allow any delayed tasks to be scheduled
    await asyncio.sleep(0.2)

    # Verify that the connection was NOT closed for successful authorization
    assert not mock_connector.close.called, "Connection should NOT be closed for successful authorization"
...
if __name__ == "__main__":
    ...
    async def run_tests():
        await test_node_attach_frame_handler_no_longer_closes_connections()
