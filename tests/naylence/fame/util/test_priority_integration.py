...
class BasicTestFactory(TestResourceFactory):
    ...
    async def create(self, config: Optional[ResourceConfig | dict[str, Any]] = None, **kwargs: Any) -> TestResource:
...
async def test_create_default_resource():
    ...
    mgr = ExtensionManager.lazy_init(
        group="naylence.TestResourceFactory", 
        base_type=TestResourceFactory
    )
    ...
    if resource.name == "advanced":
        ...
        print(f"❌ create_default_resource selected wrong factory (resource.name: {resource.name}), expected 'advanced'")
        return False
...
async def test_create_default_resource_with_config():
    ...
    # Set up the extension manager
    mgr = ExtensionManager.lazy_init(
        group="naylence.TestResourceFactoryWithConfig", 
        base_type=TestResourceFactory
    )
    ...
    if resource.name == "advanced":
        ...
        print(f"❌ create_default_resource with config selected wrong factory (resource.name: {resource.name}), expected 'advanced'")
        return False
