...
from naylence.fame.delivery.delivery_tracker import EnvelopeStatus, TrackedEnvelope
...
class TestDeliveryTrackerEdgeCases:
    ...
    async def test_wait_for_pending_acks_exception_handling(self, tracker):
        ...
        envelope = create_fame_envelope(frame=DataFrame(payload="test"), to="node@test-destination")
    ...
    async def test_on_ack_nack_frames(self, tracker):
        ...
        original_envelope = create_fame_envelope(
            frame=DataFrame(payload="test"), to="node@test-destination"
        )
        ...
        await tracker.on_envelope_delivered("test-inbox", nack_envelope)
    ...
    async def test_on_correlated_message_path(self, tracker):
        ...
        corr_envelope = create_fame_envelope(
            frame=DataFrame(payload="test"), to="node@test-destination", corr_id="correlation-123"
        )
        ...
        await tracker.on_envelope_delivered("test-inbox", corr_envelope)
    ...
    async def test_error_scenarios_in_ack_handling(self, tracker):
        ...
        create_fame_envelope(frame=DataFrame(payload="test"), to="node@test-destination")

        # Create ACK envelope
        ack_envelope = create_fame_envelope(
            frame=DeliveryAckFrame(ok=True, ref_id="original-envelope"), to="node@test-destination"
        )
