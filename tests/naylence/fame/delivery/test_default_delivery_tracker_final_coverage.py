...
class TestDefaultDeliveryTrackerFinalCoverage:
    ...
    async def test_ack_frame_without_ref_id(self, tracker):
        ...
        ack_envelope = create_fame_envelope(
            frame=DeliveryAckFrame(ok=True, ref_id=None),
    ...
    async def test_on_correlated_message_creates_tracked_envelope(self, tracker):
        ...
        envelope = create_fame_envelope(
            frame=DataFrame(payload={"test": "correlation"}),
            to=FameAddress("test@/correlation")
        )
    ...
    async def test_await_envelope_already_expired_timeout(self, tracker):
        ...
        envelope = create_fame_envelope(
            frame=DataFrame(payload={"test": "expired"}),
            to=FameAddress("test@/expired")
        )
    ...
    async def test_send_ack_full_execution_path(self, tracker):
        ...
        envelope = create_fame_envelope(
            frame=DataFrame(payload={"test": "send_ack"}),
            to=FameAddress("test@/send_ack")
        )
        envelope.reply_to = FameAddress("test@/reply")
    ...
    async def test_track_duplicate_correlation_for_replies(self, tracker):
        ...
        envelope1 = create_fame_envelope(
            frame=DataFrame(payload={"test": "first"}),
            to=FameAddress("test@/first")
        )
        ...
        envelope2 = create_fame_envelope(
            frame=DataFrame(payload={"test": "second"}),
            to=FameAddress("test@/second")
        )
    ...
    async def test_send_ack_node_send_execution(self, tracker):
        ...
        envelope = create_fame_envelope(
            frame=DataFrame(payload={"test": "node_send"}),
            to=FameAddress("test@/node_send")
        )
    ...
    async def test_await_envelope_with_zero_remaining_timeout(self, tracker):
        ...
        envelope = create_fame_envelope(
            frame=DataFrame(payload={"test": "zero_timeout"}),
            to=FameAddress("test@/zero_timeout")
        )
    ...
    async def test_await_ack_with_tracked_envelope_timeout(self, tracker):
        ...
        envelope = create_fame_envelope(frame=DataFrame(payload="test_content"), to="test@/destination", trace_id="trace123")
    ...
    async def test_on_ack_original_envelope_same_id(self, tracker):
        ...
        original_envelope = create_fame_envelope(frame=DataFrame(payload="test_content"), to="test@/destination", trace_id="trace123", corr_id="corr123")
    ...
    async def test_on_stream_item_no_queue(self, tracker):
        ...
        response_envelope = create_fame_envelope(frame=DataFrame(payload="response_content"), to="test@/response")
    ...
    async def test_wait_for_pending_acks_no_tracked_envelope(self, tracker):
        """Test wait_for_pending_acks when tracked envelope not found."""
        from unittest.mock import AsyncMock
        ...
        # Add to pending acks but don't create tracked envelope
        async with tracker._lock:
            tracker._ack_futures[envelope_id] = future
        
        # Mock outbox to return None for get()
        original_outbox = tracker._outbox
        mock_outbox = AsyncMock()
        mock_outbox.get = AsyncMock(return_value=None)
        tracker._outbox = mock_outbox
        
        # Should handle missing tracked envelope gracefully
        try:
            await tracker._wait_for_pending_acks()
        except Exception:
            pass  # Some exceptions may occur, but shouldn't crash
        finally:
            tracker._outbox = original_outbox
    ...
    async def test_heartbeat_sent_with_show_envelopes(self):
        ...
        try:
            ...
            test_tracker = tracker_module.DefaultDeliveryTracker(
    ...
    async def test_await_ack_with_none_timeout_ms(self, tracker):
        ...
        # Create ack envelope  
        ack_envelope = create_fame_envelope(
            frame=DeliveryAckFrame(ref_id=envelope.id),
