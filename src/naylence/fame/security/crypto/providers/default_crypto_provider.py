...
class DefaultCryptoProvider(CryptoProvider):
    ...
    @classmethod
    def with_ca_files(cls, ca_cert_file: str, ca_key_file: str, **kwargs) -> DefaultCryptoProvider:
        """
        Create a DefaultCryptoProvider instance.

        Note: CA certificate files are no longer used for self-signing.
        This method is maintained for API compatibility but the CA files
        are ignored since certificate generation now uses external CA services.

        Args:
            ca_cert_file: Path to CA certificate PEM file (ignored)
            ca_key_file: Path to CA private key PEM file (ignored)
            **kwargs: Additional arguments passed to DefaultCryptoProvider constructor

        Returns:
            Configured DefaultCryptoProvider instance
        """
        logger.warning(
            "with_ca_files_deprecated",
            message="CA files are ignored - use external CA service for certificate provisioning",
        )
        return cls(**kwargs)

    @classmethod
    def with_ca_pems(cls, ca_cert_pem: str, ca_key_pem: str, **kwargs) -> DefaultCryptoProvider:
        """
        Create a DefaultCryptoProvider instance.

        Note: CA certificate PEMs are no longer used for self-signing.
        This method is maintained for API compatibility but the CA PEMs
        are ignored since certificate generation now uses external CA services.

        Args:
            ca_cert_pem: CA certificate as PEM string (ignored)
            ca_key_pem: CA private key as PEM string (ignored)
            **kwargs: Additional arguments passed to DefaultCryptoProvider constructor
        ...
        """
        logger.warning(
            "with_ca_pems_deprecated",
            message="CA PEMs are ignored - use external CA service for certificate provisioning",
        )
        return cls(**kwargs)

    def _get_ca_credentials(self) -> tuple[str, str]:
        """
        DEPRECATED: Get CA credentials.

        This method is deprecated and only exists for API compatibility.
        It returns a test CA for backward compatibility with existing test code.

        In production, use external CA services for certificate provisioning.

        Returns:
            Tuple of (ca_cert_pem, ca_key_pem) for test CA
        """
        logger.warning(
            "_get_ca_credentials_deprecated",
            message="This method is deprecated - use external CA service",
        )

        # Return test CA for compatibility
        from naylence.fame.security.cert.ca_service import create_test_ca

        return create_test_ca()

    def _ensure_test_certificate(self) -> None:
        """
        Helper method for test compatibility.

        Generates a certificate using the CA service if context is set and no certificate exists.
        This is only used by tests that expect automatic certificate generation.
        """
        if not self._cert_context or self._node_cert_pem:
            return
        ...
        try:
            ...
            cert_pem = ca_service.sign_node_cert(
                public_key_pem=self._signature_public_pem,
                node_id=self._cert_context["node_id"],
                node_sid=self._cert_context["node_sid"],
                physical_path=self._cert_context["physical_path"],
                logicals=self._cert_context["logicals"],
            )

            # Store certificate
            self.store_signed_certificate(cert_pem)

            logger.debug(
                "test_certificate_generated",
                node_id=self._cert_context["node_id"],
                message="Generated certificate for test compatibility",
            )

        except Exception as e:
            logger.debug(
                "test_certificate_generation_failed",
                error=str(e),
